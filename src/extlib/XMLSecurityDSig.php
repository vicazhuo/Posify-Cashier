<?php
/**
 * Hi，I'm Wayen zhuo from China, and it's an honor to have my program noticed by you. If it can help you with your studies or if you're running a business, that's really nice.
 * Auther:Wayen zhuo
 * Email:wayenzhuo@gmail.com
 * DateTime: 2023/1/5 11:03
 * @return
 */

namespace AzureSaml\extlib;
  ...
  <ds:SignedInfo>
    ...
    private function getXPathObj() {
        if (empty($this->xPathCtx) && ! empty($this->sigNode)) {
            $xpath = new \DOMXPathDocument($this->sigNode->ownerDocument);
    ...
    public function locateSignature($objDoc, $pos=0) {
        ...
        if ($doc) {
            $xpath = new \DOMXPathDocument($doc);
            $xpath->registerNamespace('secdsig', XMLSecurityDSig::XMLDSIGNS);
    ...
    public function validateDigest($refNode, $data) {
        $xpath = new \DOMXPathDocument($refNode->ownerDocument);
        $xpath->registerNamespace('secdsig', XMLSecurityDSig::XMLDSIGNS);
    ...
    static function auxKeyInfo($parentRef, $xpath=null)
    {
        $baseDoc = $parentRef->ownerDocument;
        if (empty($xpath)) {
            $xpath = new \DOMXPathDocument($parentRef->ownerDocument);
            $xpath->registerNamespace('secdsig', XMLSecurityDSig::XMLDSIGNS);
        }
